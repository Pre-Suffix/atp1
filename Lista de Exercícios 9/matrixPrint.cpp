...
/**
 * Prints a pointer of pointers matrix to stdout
 * based on matrixPrint which uses one-dimensional arrays
 */
void matrixPrint2D(int **matrix, int height, int width) {
    int maxLength = height * width;
    int *sMatrix = (int *) calloc(maxLength, sizeof(int));

    for(int i = 0; i < maxLength; ++i) sMatrix[i] = matrix[i / width][i % width];

    matrixPrint(sMatrix, height, width);
}